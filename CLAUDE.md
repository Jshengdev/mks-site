...
- 2026-06-26: Real entry point is `src/main.jsx` (react-router-dom), NOT `App.jsx`. Routes: `/` = ProfessionalSite, `/experience/*` = WebGL. `vercel.json` rewrites all non-`/api` paths → `index.html`, so adding a client-side route needs zero config. Added `/compose-with-me` (+ `/compose-with-me/:date`, defaults to 2026-07-11) → `src/site/ComposeWithMe.jsx`, a full-screen iframe of `michaelkimsheng.engram.sh/compose-with-me/<date>` so the URL stays on our domain while engram collects fan data. Engram sends no X-Frame-Options/CSP, so framing works. Verified live in headless Chromium (form renders, 200).
- 2026-06-26: DNS for michaelkimsheng.com is hosted on **Vercel** (`ns1/ns2.vercel-dns.com`), registrar is third-party. Manage records with `vercel dns ls/add michaelkimsheng.com` (CLI linked, team `johnny-shengs-projects`). Contact-form email uses Resend (`resend._domainkey`, `send` TXT/MX). Engram's email uses SendGrid — added 3 CNAMEs: `em4608`, `s1._domainkey`, `s2._domainkey` → `*.u44828750.wl001.sendgrid.net`. The two email setups are independent.
